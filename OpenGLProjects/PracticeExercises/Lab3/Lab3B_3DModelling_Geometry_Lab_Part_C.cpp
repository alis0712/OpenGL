...
/*
Syed Ali
COSC 518 Computer Graphics Lab 3B
3D Geometry Lab

 tasks:
 1) Run the program
 2) try changing viewer location so you can see the objects from the top of the scene.
 3) change the size of these 3D objects ( cube and torus – bigger, cone, sphere – smaller ).
 */
...
main(int argc, char** argv)
{
	...
	glutSolidSphere(0.70, 32, 32);
	glEndList();

	// The cone.
	glNewList(lists + 3, GL_COMPILE);
	glutSolidCone(0.90, 1.90, 32, 32);
	glEndList();

	// The torus.
	glNewList(lists + 4, GL_COMPILE);
	glutSolidTorus(0.38, 1.2, 32, 32);
	glEndList();
